...
fn randomized(
    ...
    for _ in 0..9 {
        ...
        let address = w_addrs.choose(&mut rng).expect("address").clone();
        let fraction: f64 = rng.gen_range(0f64, 1f64);
        let partial_amount = BigDecimal::try_from(fraction).expect("bigdecimal") * &amount;
